...
open class DatePickerDialog: UIView {
    ...
    fileprivate var monthPicker: SRMonthPicker!
    ...
    fileprivate var isHideDay:       Bool = false
    ...
    public init(isHidenDaySelection: Bool) {
        
        self.isHideDay = isHidenDaySelection
    ...
    fileprivate func createContainerView() -> UIView {
        ...
        // month picker
        self.monthPicker = SRMonthPicker(frame: CGRect(x: 0, y: 30, width: 0, height: 0))
        ...
        if isHideDay == false {
            ...
            self.monthPicker.autoresizingMask = UIViewAutoresizing.flexibleRightMargin
            self.monthPicker.frame.size.width = 300
            dialogContainer.addSubview(self.monthPicker)
            self.monthPicker.maximumYear = 2030
            self.monthPicker.minimumYear = 1990
        }
        ...
        if sender.tag == kDatePickerDialogDoneButtonTag {
            ...
            if isHideDay == false {
                ...
                self.callback?(self.datePicker.date)
                 ...
                 self.callback?(self.monthPicker.date)
